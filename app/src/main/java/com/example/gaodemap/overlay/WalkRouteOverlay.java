...
/**
 * 步行图层类，在高德的API里，如果要显示步行路线规划，也可以使用此类来创建步行路线图层，
 * 如不满足需求，也可以自己创建自定义的步行路线图层。
 */
public class WalkRouteOverlay extends RouteOverlay {
     ...
     *                <strong><a href="../../../../../../Search/com/amap/api/services/route/WalkStep.html" title="com.amap.api.services.route中的类">WalkStep</a></strong>。
     * @param start   起点。详见搜索服务模块的核心基础包（com.amap.api.services.core）中的类
     *                <strong><a href="../../../../../../Search/com/amap/api/services/core/LatLonPoint.html" title="com.amap.api.services.core中的类">LatLonPoint</a></strong>。
    ...
    private void checkDistanceToNextStep(WalkStep walkStep, WalkStep walkStep1) {
        LatLonPoint lonPoint = getLastWalkPoint(walkStep);
        LatLonPoint nextFirstPoint = getFirstWalkPoint(walkStep1);
        if (!(lonPoint.equals(nextFirstPoint))) {
            addWalkPolyLine(lonPoint, nextFirstPoint);
        }
    }
    ...
    private void addWalkPolyLine(LatLng latFrom, LatLng latTo) {
        mPolylineOptions.add(latFrom, latTo);
    }
    ...
    private LatLonPoint getLastWalkPoint(WalkStep step) {
        return step.getPolyline().get(step.getPolyline().size() - 1);
    }

    private void addWalkPolyLines(WalkStep walkStep) {
        mPolylineOptions.addAll(AMapServicesUtil.convertArrayList(walkStep.getPolyline()));
    }
